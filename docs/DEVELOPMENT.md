...
项目第一阶段已经完成，并已打 `v0.1.0` 标签：核心离线评测流水线可通过 CLI 运行，默认数据随包发布，阶段一 Markdown/JSON 交付报告已提交，并有测试验证交付报告与当前 pipeline 输出一致。当前处于第二阶段，重点是鲁棒性与可解释性增强；已完成鲁棒性样本、检测器边界测试、规则元数据模型和规则命中摘要。
...
- Markdown：面向人工审阅，包含指标汇总、分类结果、规则命中摘要、漏检/误报/高风险案例。
- JSON：面向自动化验证，包含逐条结果、规则命中摘要、metrics、false positives、false negatives。
