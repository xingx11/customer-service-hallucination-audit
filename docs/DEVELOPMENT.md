...
当前处于第三阶段功能开发：阶段三调整为 Adapter + 最小 LLM 接入闭环，已完成版本/发布元数据一致性、detector adapter contract、deterministic adapter 和 mock adapter，后续继续接入 LLM 输出 schema 和显式 opt-in 的 LLM adapter。默认路径继续保持离线、确定性、无真实 LLM API 依赖。
...
- 下一步建议做 Task 21，增加 LLM 输出 schema、prompt 模板和解析校验。
