...
当前第四阶段已经完成：阶段四只做最终交付收尾，已完成 `v1.0.0` 发布准备，包括最终文档复核、发布检查清单、质量门禁、安装/CLI smoke test、阶段四交付报告、`1.0.0` 版本元数据和最终标签步骤说明。阶段四没有新增 detector 能力，没有新增运行时依赖，也没有加入 `.env` 自动加载。复核记录见 `docs/DELIVERY_REVIEW.md`，质量记录见 `docs/QUALITY_SMOKE.md`，阶段四交付报告见 `docs/reports/stage-4-report.md` 和 `docs/reports/stage-4-report.json`。
...
- 当前计划入口：`tasks/plan.md` 和 `tasks/todo.md` 已标记第四阶段完成；阶段四专属文档为 `tasks/stage-4-plan.md` 和 `tasks/stage-4-todo.md`，发布检查清单为 `docs/RELEASE_CHECKLIST.md`，交付复核记录为 `docs/DELIVERY_REVIEW.md`，质量 smoke 记录为 `docs/QUALITY_SMOKE.md`。
...
- 阶段四最终文档与交付复核
- 阶段四最终质量门禁与安装/CLI smoke test
- 阶段四 Markdown/JSON 交付报告
- `1.0.0` 版本元数据与发布记录

待完成：

- 合并本收尾 PR 到 main 后，按 `docs/RELEASE_CHECKLIST.md` 打 `v1.0.0` annotated tag。
...
- 质量 smoke 记录：`docs/QUALITY_SMOKE.md`
- 阶段四报告：`docs/reports/stage-4-report.md` 和 `docs/reports/stage-4-report.json`
- 下一步建议合并本 PR 后打 `v1.0.0` 标签。
