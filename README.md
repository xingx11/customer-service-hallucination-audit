...
当前状态：第一阶段离线评测流水线已经完成并打 `v0.1.0` 标签；阶段二正在推进鲁棒性与可解释性增强，已完成鲁棒性样本、检测器边界测试、规则元数据和规则命中摘要。默认命令可读取随包数据并生成 Markdown/JSON 报告；阶段一交付报告已提交在 `docs/reports/` 下，并随当前流水线格式保持一致。
...
- `report.md`：面向人工审阅的指标汇总、逐条分类结果、规则命中摘要和错误分析。
- `report.json`：面向自动化消费的结构化检测结果、规则命中摘要、指标和误判分组。
...
检测方法是离线确定性规则：每条规则只读取用户问题、系统回复和知识库文本，通过关键词组合判断是否触发，并输出规则 ID、幻觉类型和中文原因。规则元数据包含稳定 ID、风险等级、中文说明和触发意图，用于报告中的规则命中摘要。人工真值只用于评估 precision、recall、F1 和错误分析，不参与预测。
...
- 规则依赖文本关键词和知识库片段，对同义改写、复杂多轮上下文和隐含事实的泛化能力有限。
- 当前默认数据集只有 20 条样本，指标用于阶段一验证，不代表真实线上分布。
