...
当前状态：第一阶段离线评测流水线已经完成并打 `v0.1.0` 标签；第二阶段鲁棒性与可解释性增强已完成并打 `v0.2.0` 标签；第三阶段评测扩展与适配边界规划已启动。默认命令可读取随包数据并生成 Markdown/JSON 报告；阶段一和阶段二交付报告均已提交在 `docs/reports/` 下，并随当前流水线格式保持一致。
...
第三阶段将当前离线评测流水线扩展为更容易演进的评测框架，优先处理：

- 版本元数据与发布记录对齐。
- detector adapter contract，默认仍使用确定性规则检测器。
- 评测套件模型，用显式元数据组合多个 replies/ground truth 文件。
- 跨套件报告元数据和离线报告回归比较。

真实 LLM API 不在默认路径中；如需接入，必须作为独立设计和评审项。
